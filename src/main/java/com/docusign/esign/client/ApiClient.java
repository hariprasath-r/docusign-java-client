...
public class ApiClient {
	...
	public ApiClient(String oAuthBasePath, String[] authNames) {
		...
		for (String authName : authNames) {
			...
			if (authName == "docusignAccessCode") {
				auth = new OAuth(httpClient, OAuthFlow.accessCode, oAuthBasePath + "/oauth/auth",
						oAuthBasePath + "/oauth/token", "all");
	 ...
	 *            The API base path
	 * @param authName
	 *            the authentication method name ("oauth" or "api_key")
	 * @param clientId
	 *            OAuth2 Client ID
	 * @param secret
	...
	/**
	 * Helper method to preset the OAuth access token of the first OAuth found
	 ...
	 * the authentications (there should be only one)
	 * 
	 * @return
	 */
	...
	/**
	 * Helper method to configure authorization endpoint of the first oauth
	 * found in the authentications (there should be only one)
	 * 
	 * @return
	 */
	...
	/**
	 * Helper method to build the OAuth JWT grant uri (used once to get a user
	...
	public String getJWTUri(String clientId, String redirectURI, String oAuthBasePath) {
		return UriBuilder.fromUri(oAuthBasePath).scheme("https").path("/oauth/auth").queryParam("response_type", "code")
				.queryParam("scope", "signature%20impersonation").queryParam("client_id", clientId)
				.queryParam("redirect_uri", redirectURI).build().toString();
	}

	/**
	 * Configures the current instance of ApiClient with a fresh OAuth JWT
	 * access token from DocuSign
	 * 
	 ...
	 * @param oAuthBasePath
	 *            DocuSign OAuth base path (account-d.docusign.com for the
	 *            developer sandbox and account.docusign.com for the production
	 *            platform)
	 * @param clientId
	 ...
	 * @throws IOException
	 *             if there is an issue with either the public or private file
	 * @throws ApiException
	 *             if there is an error while exchanging the JWT with an access
	 *             token
	 */
	public void configureJWTAuthorizationFlow(String publicKeyFilename, String privateKeyFilename, String oAuthBasePath,
			String clientId, String userId, long expiresIn) throws IOException, ApiException {
		try {
			String assertion = JWTUtils.generateJWTAssertion(publicKeyFilename, privateKeyFilename, oAuthBasePath,
					clientId, userId, expiresIn);
	...
	public UserInfo getUserInfo(String oAuthBasePath) throws ApiException {
		try {
			String accessToken = this.getAccessToken();
			if (accessToken == null) {
				throw new ApiException("Cannot find a valid access token. Make sure OAuth is configured before you try again.");
			}
			
			Client client = Client.create();
			WebResource webResource = client.resource("https://" + oAuthBasePath + "/oauth/userinfo");
			ClientResponse response = webResource.header("Authorization", "Bearer " + accessToken).get(ClientResponse.class);
			ObjectMapper mapper = new ObjectMapper();
			UserInfo userInfo = mapper.readValue(response.getEntityInputStream(), UserInfo.class);
		...
		if (response.getStatusInfo().getFamily() != Family.SUCCESSFUL) {
			...
			throw new ApiException(response.getStatusInfo().getStatusCode(),
					"Error while requesting server, received a non successful HTTP code "
							+ response.getStatusInfo().getStatusCode() + " with response Body: '" + respBody + "'",
